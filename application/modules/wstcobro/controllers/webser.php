...
$array_1["saldo_deuda_o_saldo_capital"]=str_replace(',','.',$datos[9]); $array_1["plazo_original_credito"]=$datos[10];
...
$array_1["total_cuotas_vencidas"]=str_replace(',','.',$datos[15]); $array_1["fecha_ultimo_pago"]=$datos[16];
...
$array_1["agencia"]=$datos[25]; $array_1["gestor"]="0"; $array_1["codigo"]="0"; $array_1["correo"]=$datos[26]; $array_1["cliente"]=$datos[27];
return $array_1;
}
    ...
    function ingresarDatos2($matriz_completa) {
	  ...
	  $cliente=$matriz_completa["cliente"];
     ...
     */
	  ...
	  $newdatetime = str_replace('/', '-', $fecha_adjudicacion);
      $credit_detail->adjudicacion_date = $newdatetime;
	  ...
	  $newdatetime1 = str_replace('/', '-', $last_pay_date);
      $credit_detail->last_pay_date = $newdatetime1;
    ...
    function save_client_references($matriz_completa, $client_id) {
        ...
        if ($res_deudor) {
            $telefonos_deudor = $matriz_completa["telefono_cliente"];
             if (!empty($telefonos_deudor)) {
                ...
                foreach ($array_phone as $phone_contact) {
					 if (strlen($phone_contact)>6 AND strlen($phone_contact)<10) {
			...
			$correo=$matriz_completa["correo"];
            if (strlen($correo)>8) {
                $this->_save_contact($res_deudor->person_id, $correo, 3,2);
            }
        }
        /**
         * Almacena garantes
		...
		if(!empty($nombres_garante)){
            ...
            $array_telefonos = explode(';', $telefonos_garante);
				...
				foreach ($array_telefonos as $phone_contact) {
					 if (strlen($phone_contact)>6 AND strlen($phone_contact)<10) {
                    $this->_save_contact($res_garante->person_id, $phone_contact, 1,2);
						}
						if (strlen($phone_contact)>9) {
                    $this->_save_contact($res_garante->person_id, $phone_contact, 2,2);
						}
				}
            ...
            for($i=0; $i<=count($array_nombre_referencia); $i++){
                ...
                if ($res_ref2) { 
                    ...
                    if (strlen($telefono_referencia_contact)>6 AND (strlen($telefono_referencia_contact)<9 ) ) {
                        $this->_save_contact($res_ref2->person_id, $telefono_referencia_contact, 1,2);       
                    }
					if (strlen($telefono_referencia_contact)>9  ) {
                        $this->_save_contact($res_ref2->person_id, $telefono_referencia_contact, 2,2);       
                    }
    ...
    function _save_contact($person_id, $contact, $contact_type, $contact_respuesta) {
...
public function actualiza_get() {
	...
	$fecha= date('Ymd');
	//$fecha= '20190218';
	$fp = fopen("C:\TCOBRO\pagos-".$fecha.".txt", "r");
	if ($fp) {
	while (!feof($fp)) {
    $line = fgets($fp);
	$array = explode("|",utf8_encode($line));
				$credito=trim($array[0]);
			$pago=str_replace(',','.',$datos[1]);
	...
	fclose($fp);
       ...
       public function cancelado_get(){
$mysqli = new mysqli($this->db->hostname,$this->db->username,$this->db->password,$this->db->database);

if (!$mysqli->multi_query("select max(load_date) from credit_detail")){
...
$fecha=$coco[0][0];
//porque es fecha de carga diferente a la maxima
if (!$mysqli->multi_query("UPDATE `credit_detail` SET `credit_status_id`='22' WHERE  `load_date`!='$fecha'")){
            echo "Fallo la llamada: (" . $mysqli->errno . ") " . $mysqli->error;
    }
}
