...
class Credit_detail_model_of extends \CI_Model {
    
    private function _get_credit_detail_filter( $filter = null, $client_id,$oficial) {
            $this -> db -> where ('c_d.company_id', $this->user->company_id);
			//$this -> db -> where ('c_d.credit_status_id !=', 22);
			//$this -> db -> where('c_d.oficial_credito_id', 7428);
            $this -> db -> where_in('c_d.oficial_credito_id',unserialize(urldecode(stripslashes($oficial))));
            if($client_id>0){
        ...
        public function get_credit_detail_count( $filter = null, $client_id,$oficial) {  
            $this->_get_credit_detail_filter($filter, $client_id,$oficial);
