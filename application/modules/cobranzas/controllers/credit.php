...
class Credit extends CI_Controller {
	function __construct() {
        ...
        //para ver los juicios por oficial
        public function get_credit_detail_report_l(){
            $this->load->model('credit_detail_model_l');
            $sort = $this->input->get('sort');
            $order = $this->input->get('order');
            $limit = $this->input->get('limit');
            $offset = $this->input->get('offset');
            $filter = json_decode($this->input->get('filter'));
            $order_by = array($sort=>$order);
            $res = $this->credit_detail_model_l->get_credit_detail_data( $limit, $offset, $filter, $order_by);
            $total = $this->credit_detail_model_l->get_credit_detail_count( $filter);
            echo '{"total": '.$total.', "rows":'.json_encode($res).'}';
        }
		
		
			public function get_cuotas($client_id){
            ...
            $offset = $this->input->get('offset');
            $order_by = array($sort=>$order);
            $res = $this->cuotas_model->get_cuotas_data($limit, $offset, $order_by, $client_id);
			$total = $this->cuotas_model->get_count($limit, $offset, $order_by, $client_id);
			echo '{"total": '.$total.', "rows":'.json_encode($res).'}';
        }
		...
		public function get_alertas(){
            ...
            $filter = json_decode($this->input->get('filter'));
            $order_by = array($sort=>$order);
            $res = $this->alertas_model->get_alertas_data( $limit, $offset, $filter, $order_by);
            $total = $this->alertas_model->get_alertas_count( $filter);
            echo '{"total": '.$total.', "rows":'.json_encode($res).'}';
        }
	...
	public function get_credit_detail_report_f($client_id,$max,$min){
		...
		//FILTRO OFICIALES
		public function get_credit_detail_of($client_id,$oficial){
            $this->load->model('credit_detail_model_of');
            $sort = $this->input->get('sort');
            $order = $this->input->get('order');
            $limit = $this->input->get('limit');
            $offset = $this->input->get('offset');
            $filter = json_decode($this->input->get('filter'));
            $order_by = array($sort=>$order);
            $res = $this->credit_detail_model_of->get_credit_detail_data( $limit, $offset, $filter, $order_by, $client_id,$oficial);
            $total = $this->credit_detail_model_of->get_credit_detail_count( $filter, $client_id,$oficial );
            echo '{"total": '.$total.', "rows":'.json_encode($res).'}';
        }
		//FILTRO AGENCIAS
		
		public function get_credit_detail_ag($client_id,$ag){
            $this->load->model('credit_detail_model_ag');
            $sort = $this->input->get('sort');
            $order = $this->input->get('order');
            $limit = $this->input->get('limit');
            $offset = $this->input->get('offset');
            $filter = json_decode($this->input->get('filter'));
            $order_by = array($sort=>$order);
            $res = $this->credit_detail_model_ag->get_credit_detail_data( $limit, $offset, $filter, $order_by, $client_id,$ag);
            $total = $this->credit_detail_model_ag->get_credit_detail_count( $filter, $client_id,$ag );
            echo '{"total": '.$total.', "rows":'.json_encode($res).'}';
        }
        ...
        */
        ...
        function open_credit_detaill($id = 0) {
            $res['id'] = $id;
            $this->load->view("credit_detaill", $res);
        }
        ...
        public function save(){
			...
			//--------------------------------------MOTIVO NO PAGO
			$credit_detail->comision_id=set_post_value('motivo_no_pago_id');
            $credit_detail->fecha_ultima_gestion= date('Y-m-d', time());
			$credit_detail->cliente=set_post_value('promesa');
...
//                    $this->_save_ref_client( $credit_detail->client_id, 2 );
        ...
        private function _save_credit_hist($credit_detail_id) {
            ...
            $credti_hist->comision_id=set_post_value('motivo_no_pago_id');
            
            $resul=$this->db->query("SELECT credit_detail_id FROM credit_hist WHERE credit_detail_id=$credit_detail_id and credit_status_id=23");
            $lol=$resul->num_rows();
			...
			////----------------------ALERTAS---VOLVER A LLAMAR------------
			
            if (set_post_value('credit_status_id') == 13) {
                $alertas = new gestcobra\alertas_model();
			$alertas->usuario = $this->user->id;			
			$credito = (new gestcobra\credit_detail_model())
                    ->where('id',$credit_detail_id)
                    ->find_one();
					$persona = (new gestcobra\person_model())
                    ->where('code',$credito->nro_cuotas)
                    ->find_one();
			$alertas->numero_operacion= $persona->firstname;
			$alertas->fechahora= set_post_value('fecha_llamar').' '.set_post_value('hora_llamar');
            $alertas->save();
            ...
            if($contacto_seleccionado && $message){
                ...
                foreach ($contacto_seleccionado as $contactIs){
                    ...
                    foreach($tipo_sms as $value_sms){
                     ...
                     //   $mensajes=array();
                        $contactos=array();
					//	print_r("datos".$value_type."--".$value_sms);
                        if (!$value_type == '') {
                        if ($value_sms == 'com_sms') {
                             //   $contact_completo='593'.$nuevo_numero_telefono;
                              //  array_push($mensajes, $mensaje_aux);
                                //array_push($mensaj, $contact_completo);
								$numeros ["mobile"] = $nuevo_numero_telefono;
								array_push($contactos, $numeros);
								print_r("--".$contact_completo);
                                if (!empty($mensaje_aux)) {
									
                			$message   = array(
          "country"=>"593",
            "message"=>$mensaje_aux,
             "addresseeList"=>$contactos
               );
			   
			   print_r($message);
                                $obj_commws->http_conn_comunication_cd($message);
						...
						if ($value_sms == 'com_email') {									
								$resp=$this->mail($nuevo_numero_telefono, $mensaje_aux);
                                if($resp==true){$status=1;}else{$status=-1;}
                                if($mensaje_format==-1){
									$mensaje_format=1;
								$this->save_comunication_ac($value_sms, $deudor_referencias->id, $status, $nuevo_numero_telefono,$mensaje_format);
								//$this->save_hist($credit_detail_id);
 ...
 private function save_comunication_ac($type, $referencia, $status, $contact, $notification_format_id) {
		...
		function mail($correo_receptor, $mensaje){
			...
			$correo_emisor=  'gestion.cobranzas@corporacioncentro.com';
            $asunto='Aviso Cobranza';
			$clave_correo_emisor=  '9FTpgDTy3VvD';
            $this->load->library("email");
            //configuracion para gmail
            $configGmail = array(
                'protocol' => 'smtp',
                'smtp_host' => 'box5739.bluehost.com',
                'smtp_port' => 587,
