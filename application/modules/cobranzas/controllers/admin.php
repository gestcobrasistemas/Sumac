...
class Admin extends MX_Controller {
    ...
    function load_credits_data() {
        ...
        if (file_exists($logo_path . $upl_data['file_name'])) {
            ...
            for ($x = 2; $x <= $PHPExcel->getActiveSheet()->getHighestRow(); $x++) {
                ...
                //reiniciar total_pagar=0
                  //Manejo de Abonos
                //campo total_pagar se reinicia cuando el estatus=CANCELADO 
                //y es subido nuevamente el sig. mes
                if ($credit_detail->credit_status_id==22) {
                    
                    $credit_detail->total_pagar=0;
                    
                } else {
                    $abono = $credit_detail->total_pagar;
                    
                $credit_detail->total_pagar=$abono+($credit_detail->total_cuotas_vencidas-$total_cuotas_vencidas);
    ...
    private function save_client_references($PHPExcel, $x, $credit_detail_id) {
        ...
        if ($res_deudor) {
            ...
            if (strlen($telefonos_deudor) > 5) {
                $this->_save_contact($res_deudor->person_id, $telefonos_deudor, 1, 2);
            }
            $celular_deudor = get_value_xls($PHPExcel, $this->abecedario["F"], $x);
            if (strlen($celular_deudor) > 5) {

                $this->_save_contact($res_deudor->person_id, $celular_deudor, 2, 2);
            }
        }
        ...
        if ($res_garante) {
            ...
            if (strlen($telefonos_garante) > 6) {
                ...
                $this->_save_contact($res_garante->person_id, $telefonos_garante, 2, 2);
            }
        }
        /**
        ...
        if ($res_ref2) {
            ...
            if (strlen($telefonos_referencia) > 6) {
                ...
                $this->_save_contact($res_ref2->person_id, $telefonos_referencia, 2, 2);
            }
        }
    }
...
//$mysqli = new mysqli("localhost", "coophuaq_usuario", "@tcobro@1", "coophuaq_base");
