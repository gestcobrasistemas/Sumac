...
//                array('Comunicaciones', 'icon-apple'),
        ...
        $res['content_menu'] = array(
            array('report_gest', 'Por Fechas', array('comparar' => '0')),
               //array( 'report_comunicaciones_fecha', 'Reporte de Notificaciones', array('comparar'=>'0') ),
        );
...
//            }
    ...
    public function report_by_general() {
        ...
        $this->limpiar();
        //reporte produccion de gestion
        $this->repor();
        $this->report_cumplimiento_general();
        $this->pendientes();
        $this->compromiso();
        $this->gestionados();
  ...
  public function report_by_producto() {		
  if ($this->user->role_id == 2) {           
  $agencias = (new gestcobra\oficina_company_model())   
  ->where('company_id', $this->user->company_id)              
  ->where('status', 1)                 
  ...
  ->where('oficina_company_id', 
  $this->user->oficina_company_id)            
  ->find();										 } else {    
  $agencias = (new gestcobra\oficina_company_model())             
  ->where('company_id', $this->user->company_id)           
  ->where('status', 1)          
  ...
  ->find();        }       
  $res['menu'] = array(            array('REPORTE POR PRODUCTO', 'icon-apple'),       
         );       
		 $res['content_menu'] = array(      
		 array('report_producto', 'REPORTE POR PRODUCTO', array('comparar' => '1', 'agencias' => $agencias,'oficiales' => $oficiales,)),     
                   );        $res['view_name'] = 'common/templates/left_menu';        echo $this->load->view('common/templates/dashboard', $res, TRUE);    }
...
public function report_cumplimiento_general() {
...
public function report_by_gestiones() {
        ...
        $agencias = (new gestcobra\oficina_company_model())
                ...
                ->find();
        $res['menu'] = array(
            array('REPORTE POR AGENCIAS', 'icon-book'),
        );
        $res['content_menu'] = array(
            array('report_malla', 'Reporte Por Agencias', array('comparar' => '1', 'agencias' => $agencias)),
                )
//              
        ;
        $res['view_name'] = 'common/templates/left_menu';
