...
left join oficina_company of on cd.oficina_company_id=of.id
left join credit_status cs on ch.credit_status_id=cs.id
left join oficial_credito oc on cd.oficial_credito_id=oc.id 
where (ch.hist_date>='$fechacarga' and ch.hist_date<='$fechahasta') 
...
$objActSheet->getColumnDimension('O')->setAutoSize(true);
        ...
        $objActSheet->duplicateStyle($objStyleA5, 'A1:O1'); 
        ...
        $this->excel->setActiveSheetIndex(0)
				...
				->setCellValue('O'.$cont, utf8_encode($value[14]));
