...
class Notificationformat extends CI_Controller {
    ...
    private function save_comunication($type, $creditdetail_id, $status, $cost, $contact, $network, $client_referencia, $notification_format_id) {
		...
		$comunication->motivo_no_pago_id=1;
    ...
    public function notifications_update() {
        ...
        if ($c_id_notification) {
            ...
            if ($notification_id_notification == 'notif_entregada') {
                foreach($c_id_notification as $row){
                $comunication= (new gestcobra\comunication_model())
                ->where('id',$row)
                ->find_one();
                $cliente_ref=(new gestcobra\client_referencias_model())
                ->where('id',$comunication->client_referencias_id)
                ->find_one();
                $this->db->where('id', $cliente_ref->credit_detail_id);
                $this->db->update('credit_detail', array('credit_status_id' => 17));
                        ...
                        foreach ($comunicacion_opt_client as $value_sms) {
                            ...
                            if ($value_sms == 'com_sms') {
								  ...
								  if ($message_format == -1) {$message_format = 1;}
                                 	$numero ["mobile"] = $contacto_value_nuevo;
								array_push($numeros, $numero);
							
                                if (!empty($mensaje_final)) {
                			...
                			$message   = array(
                               ...
                               if ($value_sms == 'com_email') {		
								...
								$resp=$this->mail($contacto_value_nuevo, $mensaje_final);
                                if($resp==true){$status=1;}else{$status=-1;}
                                if($message_format==-1){
									$message_format=1;
								$this->save_comunication_ac($value_type, $referencia_deudor->id, $status, $contacto_value_nuevo,$message_format);
								}
    ...
    public function save(){
        ...
        echo set_post_value('template_data');
        if ($lote == 1) {
            $this->save_update();
        } else {
